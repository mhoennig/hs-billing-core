...
internal class VatChargeModeTest {
    ...
    fun `will validate DOMESTIC country code`() {
        ...
        }.message).isEqualTo("vatCountryCode 'AT' is invalid for vatChargeMode DOMESTIC")

        assertThat(assertThrows<IllegalStateException> {
            VatChargeMode.DOMESTIC.validate(FakeConfig, "CH")
        }.message).isEqualTo("vatCountryCode 'CH' is invalid for vatChargeMode DOMESTIC")
    ...
    fun `will validate EU country code`(vatChargeMode: VatChargeMode) {
        ...
        }.message).isEqualTo("vatCountryCode 'DE' is invalid for vatChargeMode $vatChargeMode")

        assertThat(assertThrows<IllegalStateException> {
            vatChargeMode.validate(FakeConfig, "CH")
        }.message).isEqualTo("vatCountryCode 'CH' is invalid for vatChargeMode $vatChargeMode")
    ...
    fun `will validate NON_EU_REVERSE country code`() {
        ...
        }.message).isEqualTo("vatCountryCode 'DE' is invalid for vatChargeMode NON_EU_REVERSE")

        assertThat(assertThrows<IllegalStateException> {
            VatChargeMode.NON_EU_REVERSE.validate(FakeConfig, "AT")
        }.message).isEqualTo("vatCountryCode 'AT' is invalid for vatChargeMode NON_EU_REVERSE")
